...
/*   Updated: 2024/06/23 22:53:33 by misaac-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../philo.h"
//((val1.sec * 1000 + val1.usec / 1000) - (val2.sec * 1000 + val2.usec / 1000));

static void print_long_line(struct timeval actual_time, t_main_philo *philo_main, t_philo *philo, int i)
{
...
void	monitoring_function(t_main_philo *philo_main, t_philo *philo)
{
	...
	while (1 && philo_main->end_flag != 1)
	{
		...
		while (i != philo_main->nb_philo && philo_main->end_flag != 1)
		{
			...
			result = ((actual_time.tv_sec * 1000 + actual_time.tv_usec / 1000) - (philo[i].last_meal * 1000 + philo[i].last_meal_usec / 1000));
			if (result >= philo_main->t_die - 1 && result)
			{
				pthread_mutex_unlock(&philo_main->food_mtx);
				print_long_line(actual_time, philo_main, philo, i);
